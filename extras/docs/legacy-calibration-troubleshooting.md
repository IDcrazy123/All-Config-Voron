...
This compact index preserves the useful lessons from retired ToolVision, kTAMV, TKC/KCC, earlier SexBolt, and Axiscope work. The active attended SexBolt trial is documented in the StealthChanger operation guide.

## Active baseline

- SexBolt `tools_calibrate`: PF2 relative XYZ trial centered at `(80, -5.5)`, observed contact near Z12, safe approach Z18.
...
| --- | --- | --- |
| `No nozzle found`, multiple bright blobs, or scale outlier near `0.028` vs `0.041–0.044` | kTAMV/ToolVision optical ambiguity, glare, or stretched camera frame | Do not widen tolerance. Treat it as historical camera evidence; the active trial uses physical SexBolt contact. |
...
| Offset differs after cold vs 150 °C measurement | Thermal expansion and/or plastic on nozzle | Use the attended 150 °C SexBolt workflow; never mix cold and hot datasets. |
...
## KTC command ownership

`CALIBRATE_ALL_OFFSETS`, `CALIBRATE_MOVE_OVER_PROBE`, and `CALIBRATE_NOZZLE_PROBE_OFFSET` came from the readonly KTC calibration file. The active trial now supplies `[tools_calibrate]` and guarded implementations for the first two commands. `CALIBRATE_NOZZLE_PROBE_OFFSET` remains blocked because Cartographer is fixed to the shuttle.
