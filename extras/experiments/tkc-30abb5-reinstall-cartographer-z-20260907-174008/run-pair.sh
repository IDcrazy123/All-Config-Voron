...
cmd="CALIBRATE_TOOL_OFFSETS TOOLS=0,${tool} CALIBRATE_XY=0 CALIBRATE_Z=1 ALLOW_SHUTTLE_Z=1 SAVE_CONFIG=0 DRY_RUN=0 CLEAN_NOZZLE=0"
response="/tmp/tkc-30abb5-pair-${tool}-response.txt"
...
t=s.get("toolchanger", {})
r=c.get("run_record", {})
print("klipper=%s idle=%s tkc=%s phase=%s elapsed=%s physical=%s active=%s tc=%s/%s/%s" % (
    s.get("webhooks", {}).get("state"), s.get("idle_timeout", {}).get("state"),
    c.get("status"), c.get("phase"), r.get("duration_sec"),
    c.get("physical_tool"), c.get("active_tool"), t.get("status"),
    t.get("tool_number"), t.get("detected_tool_number")))
' 2>/dev/null || true)
...
echo "=== G-code events for this pair ==="
START_TIME="$start_time" curl -fsS "$api/server/gcode_store?count=250" | START_TIME="$start_time" python3 -c '
import json, os, sys
...
' 2>/dev/null || true)
    [ "$tc_state" = "ready Ready ready ${tool} ${tool}" ] && break
